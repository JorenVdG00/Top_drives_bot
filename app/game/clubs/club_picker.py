...
def pick_club_event(club_state: ClubState, stop_event):
    ...
    while event_entered == False:
        ...
        logger.debug('event_number: ' + str(event_number))
        ...
        event = evaluate_club_event(event_number, loop_number)
        if event != False:
            if stop_event.is_set():
                return False
...
def evaluate_club_event(event_number=0, loop_number=0):
    pick_score, extracted_data = evaluate_club_pick(looped_time=loop_number)
