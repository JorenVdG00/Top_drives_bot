...
# from Utils.Adb.adb_class import AdbClass
...
class TerminalTab(QWidget):
    def __init__(self, worker: Worker, main_window: 'MainWindow', parent=None):
        super().__init__(parent)
        self.worker = worker
        self.main_window = main_window
        self.adb = self.main_window.adb
        self.initUI()
    ...
    def check_adb_on_startup(self):
        ...
        adb_connected = self.worker.check_adb_connection()
        if adb_connected:
    ...
    def connect_adb(self):
        ...
        is_connected = self.worker.check_adb_connection()
        if is_connected:
            self.update_adb_status(True, "ADB is already connected.")
        else:
            self.update_adb_status(False, "ADB is not connected.")
            self.adb.adb_connector.adb_connect()
            time.sleep(1)
            self.main_window.log("Connecting ADB...")
            is_connected = self.worker.check_adb_connection()
            if is_connected:
