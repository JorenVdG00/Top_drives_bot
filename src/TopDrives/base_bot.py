...
class BotBase:
    def __init__(self):
        ...
        self.resize = ResizeClass(self.logger)
        self.screen_manager = ScreenshotManager(self.resize.resize_values, self.logger, self.cmd_runner)

        self.image_utils = ImageUtils(self.logger, self.screen_manager)
        self.file_utils = FileUtils(self.logger)
        self.extractor = ExtractorBase()

        self.adb = AdbClass(adb_ip, adb_port, self.logger, self.resize)
...
class ScreenshotManager:
    def __init__(self, resize_values: list[float, float], logger: logging.Logger, cmd_runner: AdbCommandRunner):
        super().__init__(resize_values)
