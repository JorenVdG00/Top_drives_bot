...
from basics.find_coords_ss import event_img_coords_img_path
...
def extract_name_event(image, save_dir):
    ...
    create_dir_if_not_exists(save_dir, event_name)
    print(event_name)
    # NOT USEFULL
...
def crop_event_imgs(img, save_dir):
    i = 1
    for xcoords in .values():
        x1, x2 = xcoords
        cropped_image = img.crop((x1, y1, x2, y2))
        cropped_image.save(f"{save_dir}/{name}/{event_number}-{i}.png")
        i += 1

# crop_and_save_event_type_images('./event_test/In_game', './event_test/In_game_cropped')
crop_and_save_event_type_images('../event_test_V2', 'Failed_test/event_test_V2_cropped')