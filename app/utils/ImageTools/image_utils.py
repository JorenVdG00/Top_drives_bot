...
def color_distance(c1, c2):
    return math.sqrt(sum((int(a) - int(b)) ** 2 for a, b in zip(c1[:3], c2[:3])))  # Ignore the alpha channel
...
def capture_screenshot(parent_dir=None, sub_dir=None, name=None):
    """
    Capture a screenshot of the phone screen using ADB.

    If no arguments are provided, the screenshot will be saved with a timestamp
    in the format "screenshot_%Y-%m-%d-%H-%M-%S.png" to the directory specified
    by BOT_SCREENSHOTS_DIR.

    If arguments are provided, they will be used to construct the filename as
    follows: "<parent_dir>/<sub_dir>/<name>.png".

    Args:
        parent_dir (str): The parent directory to save the screenshot to.
        sub_dir (str): The subdirectory to save the screenshot to.
        name (str): The filename to use for the screenshot.
...
def save_cv2_image(image: cv2, image_name: str,image_path: str):
    """
